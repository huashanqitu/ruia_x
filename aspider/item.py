...
class ItemMeta(type):
    ...
    def __new__(cls, name, bases, attrs):
        __fields = dict({(field_name, attrs.pop(field_name)) for field_name, object in list(attrs.items()) if isinstance(object, BaseField)})
...
class Item(metaclass=ItemMeta):
    ...
    async def get_items(cls, html: str = '', url: str = '', html_etree: etree._Element = None, **kwargs) -> list:
        ...
        if items_field:
            ...
            raise ValueError("taget_item is expected")
